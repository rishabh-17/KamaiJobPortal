...
const ListCandidates = () => {
  ...
  useEffect(() => {
    setSelected(candidates?.content?.[0]);
  }, [candidates]);
  ...
  useEffect(() => {
    if (page - 2 > 0 && page + 2 <= candidates?.totalPages) {
      setPaginationArray(Array.from({ length: 5 }, (v, k) => k + page - 2));
    } else if (page - 2 <= 0 && page + 2 <= candidates?.totalPages) {
      setPaginationArray(Array.from({ length: 5 }, (v, k) => k + 1));
    } else if (page - 2 > 0 && page + 2 > candidates?.totalPages) {
      setPaginationArray(
        Array.from({ length: 5 }, (v, k) => candidates?.totalPages - 4 + k)
      );
    } else if (page - 2 <= 0 && page + 2 > candidates?.totalPages) {
      setPaginationArray(
        Array.from({ length: 5 }, (v, k) => candidates?.totalPages - 3 + k)
      );
    }
  }, [page, candidates]);
  ...
  const responsesClick = async (jobId) => {
    try {
      ...
      if (candidateJobList.ok) {
        const candidateList = await candidateJobList.json();
        setJobPostingDetail(candidateList);
        setRegistrationAccessToken(profileToken);

        if (candidateList?.id === null) {
          navigate("/ListCandidates");
        } else {
          toast.success("Fetching Applicants list...");
          navigate("/candidateListPage"); // Show Candidate List page
        }
      } else {
