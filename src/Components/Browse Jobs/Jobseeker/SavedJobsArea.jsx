...
const ListJobs = () => {
  ...
  useEffect(() => {
    setSelected(savedJobs?.content?.[0]);
  }, [savedJobs]);
  ...
  useEffect(() => {
    ...
    if (page - 2 > 0 && page + 2 <= savedJobs?.totalPages) {
      console.log(Array.from({ length: 5 }, (v, k) => k + page - 2));
    } else if (page - 2 <= 0 && page + 2 <= savedJobs?.totalPages) {
      console.log(Array.from({ length: 5 }, (v, k) => k + 1));
    } else if (page - 2 > 0 && page + 2 > savedJobs?.totalPages) {
      console.log(
        Array.from({ length: 5 }, (v, k) => savedJobs?.totalPages - 4 + k)
      );
    } else if (page - 2 <= 0 && page + 2 > savedJobs?.totalPages) {
      console.log(
        Array.from({ length: 5 }, (v, k) => savedJobs?.totalPages - 3 + k)
      );
    }
  }, [page, searchParams]);
  ...
  const handleSave = async (jobId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setsavedJob((prevState) => ({ ...prevState, [jobId]: true }));
      } else {
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3 sm:col-span-1 flex flex-col gap-4 h-[600px] jobs-list  overflow-auto">
              ...
              <ol className="flex justify-center gap-1 text-xs font-medium">
                ...
                {Array.from({ length: savedJobs?.totalPages }, (_, i) => (
                  <li
                    className={
                      page === i + 1
