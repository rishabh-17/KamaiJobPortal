...
const AppliedJobsArea = () => {
  ...
  useEffect(() => {
    setSelected(appliedJobs?.content?.[0]);
  }, [appliedJobs]);
  ...
  useEffect(() => {
    ...
    if (page - 2 > 0 && page + 2 <= appliedJobs?.totalPages) {
      console.log(Array.from({ length: 5 }, (v, k) => k + page - 2));
    } else if (page - 2 <= 0 && page + 2 <= appliedJobs?.totalPages) {
      console.log(Array.from({ length: 5 }, (v, k) => k + 1));
    } else if (page - 2 > 0 && page + 2 > appliedJobs?.totalPages) {
      console.log(
        Array.from({ length: 5 }, (v, k) => appliedJobs?.totalPages - 4 + k)
      );
    } else if (page - 2 <= 0 && page + 2 > appliedJobs?.totalPages) {
      console.log(
        Array.from({ length: 5 }, (v, k) => appliedJobs?.totalPages - 3 + k)
      );
    }
  }, [page, searchParams]);
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3 sm:col-span-1 flex flex-col gap-4 h-[600px] jobs-list  overflow-auto">
              ...
              <ol className="flex justify-center gap-1 text-xs font-medium">
                ...
                {Array.from({ length: appliedJobs?.totalPages }, (_, i) => (
                  <li
                    className={
                      page === i + 1
